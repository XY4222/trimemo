...
Replace the current tier list in `trimemo/local_model.py` with:
...
This is the strongest argument so far for offering a `trimemo mine --classifier cloud` option for users with high-stakes archives who can accept the privacy and cost trade-off. For the default case (privacy-first, no API key required), local stays the right choice.
...
**Recommendation: do not ship `trimemo mine --mode discover` at this prompt design.** Closed-set classification with a user-defined room list stays the required path.
...
Interesting niche use: workloads where latency dominates and accuracy is forgiving. Probably not the right default for TriMemo.
...
1. **Update `trimemo/local_model.py::MODEL_TIERS` to the recommended list above.** The data justifies it and the comment can point at this report.
