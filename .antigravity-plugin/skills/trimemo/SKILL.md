---
name: trimemo
description: TriMemo — mine projects and conversations into a searchable memory palace. Use when the user asks about TriMemo, memory palace, mining memories, searching memories, palace setup, wings, rooms, or drawers; or when they want to recall past work that may already be filed in their palace.
---
...
TriMemo exposes operation-specific instructions through the CLI so this skill stays accurate as TriMemo evolves. To get instructions for any operation:
...
These are the five operations users ask for most often. Each one wraps a single TriMemo CLI subcommand. The `trimemo instructions <name>` form returns the full, version-correct guidance.

### `help` — discover what TriMemo can do
...
Use when the user has just installed TriMemo, no palace exists yet, or the user explicitly asks to set up / configure / re-initialize their palace.
...
Inside Antigravity, the TriMemo MCP server registers a rich set of tools. Use these instead of shelling out to the CLI for live operations (search, diary writes, drawer adds, knowledge graph queries, palace status). The MCP tools always reflect the current palace state without spawning a subprocess.

The MCP server is auto-registered when this plugin is installed at `~/.gemini/config/plugins/trimemo/`. If the server does not appear in Antigravity's MCP store, run `trimemo-mcp --version` to verify the binary is on PATH, then restart Antigravity.
