...
server is not connected and point them at the `trimemo` skill to set
...
1. On wake-up, the TriMemo PreInvocation hook injects verbatim palace
...
- **MCP unavailable / tool error.** Surface the error plainly and suggest
  the user verify the server (`trimemo status`, or re-run the
