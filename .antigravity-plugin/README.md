# TriMemo — Antigravity plugin

In-repo packaging for the TriMemo integration with Google's [Antigravity IDE](https://antigravity.google/).

This directory is the source of truth for what gets installed at
`~/.gemini/config/plugins/trimemo/` when the user runs the installer.
...
├── mcp_config.json        # auto-registers the trimemo-mcp stdio server
...
├── rules/
│   └── trimemo-recall.md # optional recall rule (complements the skill)
└── README.md              # this file
```
...
TriMemo can store everything, but it only helps if the agent actually
...
1. **Wake hook** (`hooks/antigravity/mempal_wake_hook_antigravity.sh`,
   ...
   model call of a conversation it runs `trimemo wake-up` and injects
...
2. **Recall skill** (`skills/trimemo-recall/SKILL.md`). The
   ...
   covers recall only; the `trimemo` skill covers setup / mine /
   status.
3. **Optional recall rule** (`rules/trimemo-recall.md`). A lightweight
   ...
   adds latency to greenfield work, honouring TriMemo's "memory should
   feel instant" budget.
