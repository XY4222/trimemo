...
contributor could eagerly import chromadb from trimemo/__init__.py and
...
class TestStartupBudget:
    ...
    def test_package_import_under_startup_budget(self):
        ...
        elapsed_ms = _measure_cold_import_ms("import trimemo")
        assert elapsed_ms < budget_ms, (
            f"'import trimemo' took {elapsed_ms:.1f}ms — "
            ...
            "trimemo/__init__.py (typically a new eager import) will break "
            "that claim. Lazy-import the heavy dependency or move the work "
            "into a function."
        )

    def test_cli_import_under_startup_budget(self):
        """``from trimemo import cli`` is the path hooks take on invocation.
        ...
        """
        budget_ms = STARTUP_BUDGET_MS * CI_MULTIPLIER
        elapsed_ms = _measure_cold_import_ms("from trimemo import cli")
        assert elapsed_ms < budget_ms, (
            f"'from trimemo import cli' took {elapsed_ms:.1f}ms — "
