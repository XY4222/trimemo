...
from trimemo.spellcheck import (
    _edit_distance,
    _get_system_words,
    _should_skip,
...
class TestShouldSkip:
    ...
    def test_camelcase_skipped(self):
        ...
        assert _should_skip("TriMemo", set()) is True
    ...
    def test_known_name_skipped(self):
        assert _should_skip("trimemo", {"trimemo"}) is True
...
def test_spellcheck_user_text_passthrough_no_autocorrect():
    ...
    with patch("trimemo.spellcheck._get_speller", return_value=None):
...
def test_spellcheck_user_text_with_speller():
    ...
    with patch("trimemo.spellcheck._get_speller", return_value=fake_speller):
        with patch("trimemo.spellcheck._get_system_words", return_value=set()):
            with patch("trimemo.spellcheck._load_known_names", return_value=set()):
...
def test_transcript_line_user_turn():
    ...
    with patch("trimemo.spellcheck.spellcheck_user_text", return_value="corrected"):
...
def test_spellcheck_transcript_processes_content():
    ...
    with patch("trimemo.spellcheck.spellcheck_user_text", return_value="fixed"):
