...
from trimemo.query_sanitizer import (
    MAX_QUERY_LENGTH,
    MIN_QUERY_LENGTH,
    SAFE_QUERY_LENGTH,
    sanitize_query,
...
class TestQuestionExtraction:
    ...
    def test_question_in_system_prompt_ignored_when_real_question_exists(self):
        ...
        real_query = "What databases does TriMemo support?"
        query = system_prompt + real_query
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
        assert "TriMemo" in result["clean_query"] or "database" in result["clean_query"].lower()
...
class TestRealWorldScenarios:
    ...
    def test_mempalace_wakeup_prepended(self):
        """Simulates trimemo wake-up output prepended to a query."""
        wakeup = (
            "TriMemo loaded. Wings: technical, emotions, identity. "
    ...
    def test_2000_char_system_prompt_with_question(self):
        ...
        real_query = "What is the status of the TriMemo project?"
        query = system_prompt + real_query
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
