...
def test_enforce_embedder_identity_skips_count_on_hnsw_divergence():
    ...
    from trimemo.palace import _enforce_embedder_identity
    ...
    with (
        patch("trimemo.embedding.current_model_name", return_value="minilm"),
        patch(
...
def test_pgvector_identity_survives_marker_rewrite(tmp_path):
    ...
    from trimemo.backends.pgvector import PgVectorBackend, _PgVectorConfig
...
def test_enforcement_match_does_not_raise(tmp_path, monkeypatch, clear_identity_cache):
    ...
    from trimemo import palace as P
...
def test_enforcement_nameless_model_is_a_noop(tmp_path, monkeypatch, clear_identity_cache):
    ...
    monkeypatch.setattr("trimemo.embedding.current_model_name", lambda model=None: "")
...
def test_set_palace_identity_empty_target_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("trimemo.config.MempalaceConfig.embedding_model", property(lambda s: ""))
    from trimemo import palace as P
...
def _qdrant_collection(tmp_path, *, write_marker=True):
    from trimemo.backends.qdrant import QdrantBackend, QdrantCollection, _QdrantConfig
...
def test_qdrant_identity_survives_marker_rewrite(tmp_path):
    from trimemo.backends.qdrant import QdrantBackend, _QdrantConfig
...
def _pgvector_collection(tmp_path, *, write_marker=True):
    from trimemo.backends.pgvector import PgVectorBackend, PgVectorCollection, _PgVectorConfig
