...
### Verify `trimemo-mcp` entry point alignment

The plugin configs reference `trimemo-mcp` as the MCP server command, which
...
```
.mcp.json:      "command": "trimemo-mcp"
pyproject.toml:trimemo-mcp = "mempalace.mcp_proxy:main"
.claude-plugin/.mcp.json:    "command": "trimemo-mcp"
.claude-plugin/plugin.json:      "command": "trimemo-mcp"
.codex-plugin/README.md:2. Install the Python package so the `trimemo-mcp` script lands on
```
...
```bash
grep -r trimemo-light-mcp pyproject.toml .dsh-plugin/cordis.patch.yml
```

Expected on a healthy `develop`:

```
pyproject.toml:trimemo-light-mcp = "mempalace.mcp_light_server:main"
.dsh-plugin/cordis.patch.yml:        command: trimemo-light-mcp
```
...
1. **PyPI trusted publisher** — on PyPI, go to **Manage project `trimemo`
   → Publishing → Add a trusted publisher** and enter exactly:

   | Field | Value |
   | --- | --- |
   | Owner | `TriMemo` |
   | Repository name | `trimemo` |
...
1. Bump the version in **all seven** sources on `develop` so `version-guard.yml`
   stays green (it is the single source of truth at `trimemo/version.py`,
...
4. On GitHub, **Releases → Draft a new release**:
   - **Target:** `main`
   - **Tag:** `vX.Y.Z` (must equal `trimemo/version.py`; the workflow and
...
5. The `publish.yml` run validates the tag (on `main`, matches the manifest),
   ...
   <https://pypi.org/project/trimemo/>.
