# Cursor Rules — TriMemo recall

Optional [Cursor rules](https://cursor.com/docs/rules) that make the
agent search TriMemo before answering questions about past work,
people, projects, or prior decisions.

These are for users who install TriMemo **without** the Cursor plugin
...
|------|---------------|------------|----------|
| [`trimemo-recall.mdc`](trimemo-recall.mdc) | `false` | Cursor's matcher decides the turn is recall-relevant (from the rule `description`) | **Recommended.** Recall without paying for the rule on unrelated work. |
| [`trimemo-recall-always.mdc`](trimemo-recall-always.mdc) | `true` | Every conversation in scope, every turn | You want recall guaranteed in context and accept the cost. |
...
latency and works against TriMemo's "memory should feel instant"
...
cp examples/cursor/rules/trimemo-recall.mdc ~/.cursor/rules/
```
...
For the aggressive variant, copy `trimemo-recall-always.mdc` instead
...
| `trimemo-recall` skill | Full search-before-answer protocol, model-invoked or attached | [`skills/trimemo-recall/`](../../../skills/trimemo-recall/) |
