...
| Row | What it does |
| --- | --- |
| `trimemo-recall` | Puts your stored memory (`trimemo wake-up`) in each session's system prompt, from the first model call |
| `trimemo-autosave` | Keeps an append-only transcript per session and runs TriMemo's own `stop`, `precompact` and `session-end` hooks against it |
| `trimemo-mcp` | Serves the palace through DSH's own MCP client with TriMemo's light server: three PQL tools, `mcp__mempalace__palace_query` and friends |
...
- TriMemo installed so that both `trimemo` and `trimemo-light-mcp` are on
  the `PATH` the harness sees, and a palace (`trimemo init`, then
  `trimemo mine`). An editable install made before `trimemo-light-mcp`
  existed will not have that script; reinstall (`pip install -e .`) to get it.
  On Windows, stop running TriMemo processes first (a hub, `logstream watch`):
...
```sh
dsh plugin --profile web add link:/absolute/path/to/trimemo/.dsh-plugin
```
...
```sh
dsh web --dump-config | grep trimemo
```

To turn one row off without uninstalling, add a patch to the profile's own
`cordis.patch.yml`, e.g. `- id: trimemo-mcp` with `disabled: true`.

## What each row does

### `trimemo-recall`

When a top-level session starts, the row runs `trimemo wake-up --wing <wing>`
once and registers a `## TriMemo memory` section on that agent. The section
serves the cached text on every model step; there is no further I/O.

- **Wing.** The wing `trimemo mine` files the project under: `wing:` from the
...
- **Verbatim.** The wake-up text is injected exactly as TriMemo printed it.
...
`$DSH_HOME/trimemo/transcripts/<session>.jsonl` (default `~/.dsh/...`).
...
- On each turn boundary it runs `trimemo hook run --hook stop --harness dsh`.
...
Everything else is TriMemo's own hook behaviour, the same as in Claude Code:
...
write routing. TriMemo's `hooks.auto_save: false` turns it off.
...
row running TriMemo's **light** MCP server, `trimemo-light-mcp`, over
...
step. Measured on TriMemo 3.9.0, that is about 2,100 tokens for the light
...
| --- | --- | --- | --- |
| `command` | both | `trimemo` | Executable, looked up on the harness `PATH` unless absolute |
| `commandArgs` | both | `[]` | Arguments before the subcommand, e.g. `command: python` + `commandArgs: ['-m', 'trimemo']` |
| `env` | both | `{}` | Extra environment for the CLI (e.g. `MEMPALACE_PALACE_PATH`) |
| `timeoutMs` | recall | `30000` | Ceiling on `trimemo wake-up` (autosave hooks get 120 s) |
...
| `transcriptDir` | autosave | `$DSH_HOME/trimemo/transcripts` | Where session transcripts are kept |
...
The plugin itself sends nothing anywhere. It runs the local `trimemo` CLI and
...
local model, or leave `trimemo-recall` and `trimemo-mcp` disabled, if that is
not acceptable.
