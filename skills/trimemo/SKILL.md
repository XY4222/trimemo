---
name: trimemo
description: Install, configure, and operate TriMemo, including a private local palace, a shared-brain hub, or a client joining an existing hub. Use for first-time setup, MCP wiring, mining, status, wings, rooms, drawers, shared-brain identity, or logstream readiness.
---

# TriMemo Setup

A guided, skill-first setup for a searchable memory palace. The user may have
installed this skill with `npx skills add` before the TriMemo Python package
...
- Run `trimemo --version`, `uv --version`, and an appropriate Python version
...
```bash
python -m pip install trimemo
```

After installation, run `trimemo --version`. If it still is not reachable,
...
TriMemo provides dynamic, version-correct instructions via the CLI. To get instructions for any operation:

```bash
trimemo instructions <command>
```
...
For a new local palace or hub, follow `trimemo instructions init`, configure
the selected corpus, then verify with `trimemo status`. For a client, skip
local initialization and obtain the hub URL and bearer token from the user.

### 5. Configure MCP

For local stdio integrations, use the command printed by `trimemo mcp`.
Typical registrations are:

```bash
claude mcp add trimemo -- trimemo-mcp
codex mcp add trimemo -- trimemo-mcp
```

For a shared-brain hub, guide the user through `trimemo serve` and the
...
tool list includes TriMemo tools. Package installation alone is not proof
...
- Render the canonical rules with:
  ...
  Default `--mcp full` matches the 45-tool `trimemo-mcp` server this
  skill registers. If the user opted into `trimemo-light-mcp`, re-render
  with `--mcp light` instead. Replace an existing
  `<!-- trimemo-shared-brain -->` block instead of appending a duplicate.
...
- Check coordination access with a read-only `trimemo logstream list` or the
...
- Interactive sessions are declared-idle: they sweep the inbox on collab /
  ...
  `trimemo logstream watch --agent <host>:<harness>:<project>` (the CLI
...
action. For active delegation, hand off to the `trimemo-task` skill.
...
`trimemo update configure --enable --installer uv-tool` (or `pipx` / `pip`);
...
their versions or installers. For a client update, use the local `trimemo
