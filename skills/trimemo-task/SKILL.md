---
name: trimemo-task
description: Create, hand off, claim, execute, and close agent tasks through the TriMemo logstream. Use when the user wants to delegate work, prepare a ready-to-paste task for another agent, receive a TriMemo task id, or explicitly launch a supported headless agent in controlled mode.
---
...
1. Confirm `trimemo --version` succeeds.
   Also confirm `trimemo task --help` and `mempalace_task_create` exist; if
   either is missing, do not assume the older runtime has update-awareness
   commands. Explain that it is incompatible, hand off to the `trimemo` setup
   skill, and propose the package-manager-appropriate upgrade (`uv tool upgrade
   trimemo`, `pipx upgrade trimemo`, or the interpreter backing the active
   TriMemo command followed by `-m pip install --upgrade trimemo`) plus
   `npx skills update trimemo trimemo-recall
   trimemo-task`. Require explicit authorization before running anything.
2. Confirm the TriMemo MCP tools are connected and the active palace is the
...
If setup is incomplete, stop and use the `trimemo` setup skill.
...
fact that delivery must close through TriMemo. Logstream events are immutable,
...
When given `Open TriMemo task <id> as <agent>...`:
...
3. Claim the request with `mempalace_event_ack(status=claimed)`. Claiming
   **is** a watch trigger: arm `trimemo logstream watch --agent <you>
...
sweep. Do not run the local SQLite `trimemo logstream watch` command unless
...
```bash
trimemo task launch <task-id> --runner codex --workspace <path>
trimemo task launch <task-id> --runner claude --workspace <path>
```
...
```bash
trimemo task launch --task-file <task-request.json> --runner codex --workspace <path>
```
...
- **MCP/logstream unavailable:** run `trimemo status` and return to setup;
