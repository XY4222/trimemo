...
class MineAlreadyRunning(RuntimeError):
    """Raised when another `trimemo mine` already holds the per-palace lock."""
...
def _validate_palace_fts5_after_mine(palace_path: str) -> None:
    ...
    instead of forcing the operator to run `trimemo repair` by hand for a
    ...
    if errors:
        ...
        # on the `trimemo mine` path -- so root keeps its default and info
...
def mine_palace_lock(palace_path: str):
    ...
    single source; it does not prevent N copies of `trimemo mine <dir>`
