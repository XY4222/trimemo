# Loaded into trimemo.palace via exec (see __init__.py). Not a standalone module.
if __name__ != "trimemo.palace":
    raise ImportError(f"{__name__} is an implementation fragment; import trimemo.palace")
    ...
    if state == "unknown" and stored is None:
        ...
        if count == 0:
            ...
            warnings.warn(
                ...
                "`trimemo palace set-embedder --model <name>` to record it.",
                EmbedderIdentityUnknownWarning,
                stacklevel=2,
            )
...
# searchable index layer and TriMemo never opens a differently-named closets
...
def _allowed_wrapper_collection_names() -> List[str]:
    ...
    Only two stores are first-class to TriMemo: the configured drawers
...
class CollectionNameMismatchError(ValueError):
    ...
    store the rest of TriMemo never reads — so data upserted through it would
    ...
    the *name* is not one TriMemo routes reads/writes through.
    """
    ...
    def __init__(self, requested: str, allowed: List[str], palace_path: Optional[str] = None):
        ...
        super().__init__(
            f"collection {requested!r} is not a TriMemo collection{where}: "
            ...
            f"TriMemo never reads, so data upserted through it would be "
...
def get_collection(
    ...
    """Get a first-class TriMemo collection (drawers or closets).
    ...
    * the **closets** collection — the searchable index layer. Always named
    ...
    Any other name points to a store the rest of TriMemo never reads, so data
...
def set_palace_embedder_identity(
    ...
    *,
    ...
    Backs ``trimemo palace set-embedder``. Returns ``(old, new)`` identities.
