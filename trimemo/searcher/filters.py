# Loaded into trimemo.searcher via exec (see __init__.py). Not a standalone module.
if __name__ != "trimemo.searcher":
    raise ImportError(f"{__name__} is an implementation fragment; import trimemo.searcher")
...
def _warn_if_legacy_metric(col) -> None:
    ...
    need ``trimemo repair`` to rebuild with the correct metric.
    ...
    print(
        ...
        "          Run `trimemo repair` to rebuild the index with the correct metric.",
        file=_sys.stderr,
    )
