...
This module is used as the primary signal in `trimemo init`. The regex
...
def _merge_detected(primary: dict, secondary: dict, drop_secondary_uncertain: bool = False) -> dict:
    ...
    Dedup is case-insensitive so "trimemo" (manifest name) absorbs "TriMemo"
...
def discover_entities(
    ...
    The expected shape is the dict written by ``trimemo init`` to
    ...
    from trimemo.convo_scanner import is_claude_projects_root, scan_claude_projects
    ...
    if is_claude_projects_root(root_path):
        ...
        # so a `pyproject.toml` name like `trimemo` and a Claude Code
        # `cwd` variant like `TriMemo` collapse into one entry — matches
    ...
    if llm_provider is not None:
        from trimemo.llm_refine import collect_corpus_text, refine_entities
    ...
    if corpus_origin is not None:
        from trimemo.entity_detector import _apply_corpus_origin
