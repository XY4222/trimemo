...
Usage (from CLI):
    trimemo repair
    trimemo repair-scan [--wing X]
    trimemo repair-prune --confirm
"""
...
def _quick_check_errors(sqlite_path: str) -> list[str]:
    ...
    so it gets the same answer rather than a traceback out of ``trimemo
    mine`` and ``trimemo repair``, neither of which guards this call.
...
def print_sqlite_integrity_abort(palace_path: str, errors: list[str]) -> None:
    ...
    print("  `trimemo repair` will not call Chroma delete_collection() because")
    ...
    print("    1. Stop all TriMemo writers / MCP clients.")
    ...
    print("    6. Re-run `trimemo repair --yes`.")
...
def index_read_recovery_guidance() -> str:
    ...
    The other thing that strands this read is a live TriMemo server or
    ...
    """
    return (
        "  If a TriMemo server or mine is still running against this palace,\n"
        ...
        "  than re-mining:\n"
        "\n"
        "      trimemo repair --mode from-sqlite --archive-existing\n"
        "\n"
...
def rebuild_index(
    ...
    progress(f"\n{'=' * 55}")
    progress("  TriMemo Repair -- Index Rebuild")
    progress(f"{'=' * 55}\n")
    ...
    * ``True`` — rename ``dest_palace`` to
    ...
    .. warning::
       ...
       *any* palace, not just this one. Do not call this function from
       inside a long-running trimemo process (MCP server, daemon)
    ...
    print(f"\n{'=' * 55}")
    print("  TriMemo Repair -- Rebuild from SQLite")
    print(f"{'=' * 55}\n")
...
def status(palace_path=None, collection_name: Optional[str] = None) -> dict:
    ...
    chromadb tries to load the undersized HNSW. Running ``trimemo
...
def repair_max_seq_id(
    ...
    *,
    ...
    print(f"\n{'=' * 55}")
    print("  TriMemo Repair -- max_seq_id Un-poison")
    print(f"{'=' * 55}\n")
    print(f"  Palace:  {palace_path}")
    if segment:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="TriMemo repair tools")
