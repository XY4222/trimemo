...
Used by trimemo init before mining begins.
...
Multi-language support:
    ...
    the ``entity`` section of ``trimemo/i18n/<lang>.json``. Every public
...
Usage:
    from trimemo.entity_detector import detect_entities, confirm_entities
...
#
# The data file lives at ``trimemo/data/coca_content_words.json``. Loaded
...
def _get_coca_filter() -> frozenset[str]:
    ...
    Loads ``trimemo/data/coca_content_words.json`` on first call and
...
#
# Data file: ``trimemo/data/known_systems.json``. Loaded once on first
...
def _get_known_systems() -> tuple[tuple[str, "re.Pattern[str]"], ...]:
    ...
    Loads ``trimemo/data/known_systems.json`` on first call, compiles a
...
# ``trimemo/i18n/en.json``. Callers that need multi-language behavior should
...
def detect_entities(
    ...
    Args:
        ...
        corpus_origin: Optional corpus-origin context (the dict produced
            ...
            ``<palace>/.mempalace/origin.json`` by ``trimemo init``).
...
def confirm_entities(detected: dict, yes: bool = False) -> dict:
    ...
    """
    print(f"\n{'=' * 58}")
    print("  TriMemo -- Entity Detection")
    print(f"{'=' * 58}")
