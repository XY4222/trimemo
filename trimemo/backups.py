...
``trimemo migrate`` and ``trimemo repair max-seq-id`` each write a fresh,
...
``trimemo repair`` in its default mode and ``trimemo migrate`` take before
...
def copy_palace_dir(src, dst, *, symlinks=False, log=None):
    ...
    skipped with its contents. Nothing in TriMemo replaces an entry that
