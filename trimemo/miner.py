...
# `trimemo mine --max-chunks-per-file N` if you hit ONNX bad_alloc on
...
def _publish_registry(registry_path, payload: dict) -> None:
    ...
    so two concurrent ``trimemo init`` runs never share one. A run killed
...
def _registry_to_merge_into(registry_path) -> Optional[dict]:
    ...
    Printed rather than logged: the one caller is ``trimemo init``, which
    ...
    try:
        ...
        # call and into ``trimemo init`` as a traceback.
        loaded = None
...
def add_to_known_entities(entities_by_category: dict, wing: str = None) -> Optional[str]:
    ...
    Accepts ``{category: [names]}`` shape as produced by ``trimemo init``
...
def process_file(
    ...
    if effective_cap > 0 and len(chunks) > effective_cap:
        ...
        # line). This keeps ``trimemo mine ... > out.log 2> err.log``
...
def _mine_impl(
    ...
    try:
        ...
        print('\n  Next: trimemo search "what you\'re looking for"')
        print(f"{'=' * 55}\n")
    except KeyboardInterrupt:
        ...
        print(
            f"\n  Re-run `trimemo mine {shlex.quote(project_dir)}` to resume — "
        ...
        print(
            f"\n  Re-run `trimemo mine {shlex.quote(project_dir)}` after addressing "
...
def _print_status(total: int, wing_rooms: dict[str, dict[str, int]]) -> None:
    ...
    print(f"\n{'=' * 55}")
    print(f"  TriMemo Status -- {total} drawers")
    print(f"{'=' * 55}\n")
