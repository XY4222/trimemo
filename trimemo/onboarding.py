#!/usr/bin/env python3
"""
onboarding.py — TriMemo first-run setup.

Asks the user:
  1. How they're using TriMemo (work / personal / combo)
...
Seeds the entity_registry with confirmed data so TriMemo knows your world
...
from trimemo.entity_registry import EntityRegistry
...
def _ask_embedding_model() -> str:
    ...
    print("""
  TriMemo embeds your memories into a vector space so the AI can find
...
def _ask_mode() -> str:
    _header("Welcome to TriMemo")
    print("""
  TriMemo is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using TriMemo?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help TriMemo distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from trimemo.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by trimemo init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
...
def run_onboarding(
  ...
  TriMemo will check the context before treating them as person names.
