...
def _write_stdout_exact(content: str) -> None:
    ...
    The counterpart to :func:`_read_stdin_exact` — ``trimemo artifact get
...
def _logstream_watch(ls, args, as_json):
    ...
    if skipped_from:
        print(
            ...
            "Use --from-start to replay them, or sweep with `trimemo logstream list`.",
            file=sys.stderr,
        )
    if recovery_failed:
        print(
    ...
    try:
        if args.artifact_action == "put":
            ...
            if as_json:
                ...
                # Exact content on stdout so `trimemo artifact get ID | git apply`
