...
def _resolve_cli_write_routing_or_exit(args, operation: str):
    ...
    try:
        ...
        print(
            f"trimemo: invalid CLI write routing: {exc}",
            file=sys.stderr,
        )
...
def _gather_origin_samples(project_dir) -> list:
    ...
    Skips trimemo's own per-project artifacts (``entities.json``,
    ``mempalace.yaml``) so a re-run of ``trimemo init`` produces the
...
def _ensure_mempalace_files_gitignored(project_dir) -> bool:
    """If project_dir is a git repo, ensure TriMemo's per-project files
    ...
    `trimemo init` writes mempalace.yaml + entities.json into the
    ...
    block = prefix + "\n# TriMemo per-project files (issue #185)\n" + "\n".join(missing) + "\n"
