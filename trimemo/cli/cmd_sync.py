...
def cmd_sync(args):
    ...
    print(f"\n{'=' * 55}")
    print("  TriMemo Sync -- Gitignore-aware drawer prune")
    print(f"{'=' * 55}")
    ...
    try:
        ...
        print(f"trimemo: {exc}", file=sys.stderr)
        sys.exit(1)
    except ValueError as exc:
        print(f"trimemo: {exc}", file=sys.stderr)
        sys.exit(2)
    except Exception as exc:
        print(f"trimemo: sync failed: {exc}", file=sys.stderr)
        sys.exit(1)
...
def _submit_daemon_cli_job(
    ...
    *,
    ...
    try:
        ...
        print(f"trimemo: daemon submission failed: {exc}", file=sys.stderr)
        sys.exit(1)
    ...
    if job_deferred_by_lock(job):
        ...
        print(f"trimemo: {reason}", file=sys.stderr)
        print(
            f"trimemo: job {job['id']} is queued and runs when the holder exits "
            f"(check it with: trimemo {palace_flag}daemon jobs)",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if job.get("state") != "succeeded" and exit_code == 0:
        ...
        print(
            f"trimemo: daemon job failed: {error.get('message', 'unknown error')}",
            file=sys.stderr,
        )
        exit_code = 1
    if exit_code:
...
def cmd_daemon(args):
    ...
    try:
        if action == "start":
            ...
            print(f"TriMemo daemon running on 127.0.0.1:{client.port}")
        ...
        if action == "stop":
            if stop_daemon(palace_path):
                print("TriMemo daemon stopping")
            else:
                print("TriMemo daemon is not running")
            return
        ...
        if action == "status":
            ...
            if client is None:
                ...
                sys.exit(1)
            health = client.health()
            print("TriMemo daemon is running")
            print(f"  Palace: {health.get('palace_path')}")
            print(f"  PID:    {health.get('pid')}")
        ...
        if action == "wait":
            ...
            exit_code = print_job_result(result)
            if job.get("state") != "succeeded" and exit_code == 0:
                print(f"trimemo: daemon job failed: {job.get('error')}", file=sys.stderr)
                exit_code = 1
            if exit_code:
                sys.exit(exit_code)
            return
    except DaemonError as exc:
        print(f"trimemo: daemon error: {exc}", file=sys.stderr)
        sys.exit(1)
