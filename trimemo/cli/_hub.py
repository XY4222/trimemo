# Loaded into trimemo.cli via exec (see __init__.py). Not a standalone module.
if __name__ != "trimemo.cli":
    raise ImportError(f"{__name__} is an implementation fragment; import trimemo.cli")
...
def _print_hub_search_result(args, result: dict) -> bool:
    ...
    if error:
        ...
        print(f"trimemo: hub search failed: {message}", file=sys.stderr)
        return False
    ...
    if result.get("vector_disabled") or result.get("fallback") == "bm25_only_via_sqlite":
        print(
            ...
            "          Run `trimemo repair` to restore vector search.\n"
        )
...
def _forward_search_to_hub(args, palace_path: str) -> bool:
    ...
    try:
        ...
        if exc.code == 401:
            ...
            return False
        print(f"trimemo: hub rejected search ({exc.code} {exc.reason})", file=sys.stderr)
        sys.exit(1)
    except (urllib.error.URLError, OSError, TimeoutError, ValueError) as exc:
        print(
            f"trimemo: hub at {base_url} did not complete the search ({exc}); "
    ...
    print(
        f"trimemo: forwarding search to palace hub {base_url} (pid {info.get('pid')})",
        file=sys.stderr,
    )
    ...
    if payload.get("error"):
        err = payload["error"]
        print(
            f"trimemo: hub refused search: {err.get('message', 'unknown error')}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        print("trimemo: hub returned an unrecognized search response", file=sys.stderr)
        sys.exit(1)
...
def _forward_mine_to_hub(args, palace_path: str) -> bool:
    ...
    A long-lived hub (``trimemo serve``) holds the MCP writer lease for
    ...
    print(f"trimemo: forwarding mine to palace hub {base_url} (pid {info.get('pid')})")
    try:
        with server_registry.urlopen_with_server_tokens(
            palace_path,
            f"{base_url}/mcp",
        ...
        print(f"trimemo: hub rejected mine ({exc.code} {exc.reason})", file=sys.stderr)
        sys.exit(1)
    except (urllib.error.URLError, OSError, TimeoutError) as exc:
        print(
            f"trimemo: hub at {base_url} did not complete the mine ({exc}); "
    ...
    if payload.get("error"):
        err = payload["error"]
        print(
            f"trimemo: hub refused mine: {err.get('message', 'unknown error')}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        print("trimemo: hub returned an unrecognized mine response", file=sys.stderr)
        sys.exit(1)
    ...
    if not result.get("success", False):
        print(f"trimemo: hub mine failed: {result.get('error', 'unknown')}", file=sys.stderr)
        sys.exit(1)
    return True
