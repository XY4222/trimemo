...
def cmd_mcp(args):
    """Show how to wire TriMemo into MCP-capable hosts."""
    base_server_cmd = "trimemo-mcp"
    ...
    print("TriMemo MCP quick setup:")
    print(f"  claude mcp add trimemo -- {server_cmd}")
    print(f"  codex mcp add trimemo -- {server_cmd}")
    print("\nLightweight MCP setup (3 consolidated tools, ~92% fewer schema tokens):")
    print(f"  claude mcp add trimemo-light -- {light_cmd}")
    print(f"  codex mcp add trimemo-light -- {light_cmd}")
    print("\nRun the server directly:")
    print(f"  {server_cmd}")

    if not args.palace:
        print("\nOptional custom palace:")
        print(f"  claude mcp add trimemo -- {base_server_cmd} --palace /path/to/palace")
        print(f"  codex mcp add trimemo -- {base_server_cmd} --palace /path/to/palace")
        print("  claude mcp add trimemo-light -- trimemo-light-mcp --palace /path/to/palace")
...
def cmd_serve(args):
    ...
    A turnkey wrapper over ``trimemo-mcp --transport http``: it resolves a
    ...
    if bool(tls_cert) != bool(tls_key):
        print("trimemo: --tls-cert and --tls-key must be given together", file=sys.stderr)
        sys.exit(2)
    for label, path in (("--tls-cert", tls_cert), ("--tls-key", tls_key)):
        if path and not os.path.isfile(path):
            print(f"trimemo: {label} file not found: {path}", file=sys.stderr)
            sys.exit(2)
    ...
    print("Starting TriMemo remote MCP server")
    ...
    if token:
        print(
            f"  claude mcp add --transport http trimemo {url} "
            f'--header "Authorization: Bearer {token if token_created else "$MEMPALACE_MCP_HTTP_TOKEN"}"'
        )
    else:
        print(f"  claude mcp add --transport http trimemo {url}")
