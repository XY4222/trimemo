...
def _reconfigure_stdio_utf8_on_windows():
    """Decode stdio as UTF-8 on Windows for the primary `trimemo` CLI.

    Thin wrapper around the shared helper in ``mempalace._stdio``. The CLI
    overrides stdout/stderr to ``replace`` because ``trimemo search``
...
def main():
    """CLI entry point for the ``trimemo`` console script.
    ...
    (``import trimemo.searcher`` from a host app) do NOT trigger this
    ...
    # trimemo/__init__.py already protects this process from the same
    # ABI mismatch; here we extend the protection to children.
    os.environ.pop("PYTHONPATH", None)

    _reconfigure_stdio_utf8_on_windows()

    version_label = f"TriMemo {__version__}"
    parser = argparse.ArgumentParser(
        description="TriMemo — Give your AI a memory. No API key required.",
    ...
    p_mine.add_argument(
        "--agent",
        default="trimemo",
        help="Your name — recorded on every drawer (default: trimemo)",
    )
    ...
    p_mine.add_argument(
        ...
        help=(
            ...
            "since `trimemo init` and the stored origin may be stale. "
            "Heuristic-only (no LLM call) — re-run `trimemo init --llm` for "
            "Tier 2 refinement."
        ),
    )
    p_mine.add_argument(
    ...
    p_rules.add_argument(
        ...
        default="full",
        help=(
            "Tool names in the block: 'full' (default, the 45-tool trimemo-mcp) or "
    ...
    p_mcp = sub.add_parser(
        "mcp",
        help="Show MCP setup command for connecting TriMemo to your AI client",
    )
    p_mcp.add_argument(
        "--backend",
        default=None,
    ...
    def _add_logstream_filters(p):
        p.add_argument("--stream", default=None, help="Stream, e.g. project/trimemo")
    ...
    p_ls_append.add_argument("--stream", required=True, help="Stream, e.g. project/trimemo")
