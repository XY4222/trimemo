#!/usr/bin/env python3
"""
TriMemo — Give your AI a memory. No API key required.

Three ways to ingest:
  Projects:      trimemo mine ~/projects/my_app                  (code, docs, notes)
  Conversations: trimemo mine <convo-dir> --mode convos          (Claude Code, Claude.ai, ChatGPT, Slack exports)
  Documents:     trimemo mine <docs-dir> --mode extract          (PDF, DOCX, PPTX, XLSX, RTF, EPUB — requires trimemo[extract])
  Adapters:      trimemo mine <source> --source <adapter-name>  (registered source adapters)

Same palace. Same search. Different ingest strategies.

Commands:
    trimemo init <dir>                  Detect rooms from folder structure
    trimemo split <dir>                 Split concatenated mega-files into per-session files
    trimemo mine <dir>                  Mine project files (default)
    trimemo mine <dir> --mode convos    Mine conversation exports
    trimemo mine <dir> --mode extract   Mine binary office documents (PDF/DOCX/etc.)
    trimemo mine <source> --source NAME Mine through a registered source adapter
    trimemo search "query"              Find anything, exact words
    trimemo mcp                         Show MCP setup command
    trimemo task create ...             Create a complete agent handoff
    trimemo task launch ...             Run a stored task headlessly
    trimemo wake-up                     Show L0 + L1 wake-up context
    trimemo wake-up --wing my_app       Wake-up for a specific project
    trimemo status                      Show what's been filed

Examples:
    trimemo init ~/projects/my_app
    trimemo mine ~/projects/my_app
    trimemo mine ~/.claude/projects/-Users-you-Projects-my_app --mode convos --wing my_app
    trimemo search "why did we switch to GraphQL"
    trimemo search "pricing discussion" --wing my_app --room costs
"""

# The public import path remains ``trimemo.cli``. Implementation is split
...
# ``from trimemo.cli import main, cmd_mine, ...`` and test monkeypatches.
