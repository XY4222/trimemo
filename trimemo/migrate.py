#!/usr/bin/env python3
"""
trimemo migrate — Recover a palace created with a different ChromaDB version.
...
Since trimemo 3.2.0 (chromadb>=1.5.4), chromadb automatically migrates
...
older trimemo release) or if automatic migration fails.

Usage:
    trimemo migrate                          # migrate default palace
    trimemo migrate --palace /path/to/palace  # migrate specific palace
    trimemo migrate --dry-run                # show what would be migrated
"""
...
def contains_palace_database(path: str) -> bool:
    """Return True when path looks like a TriMemo ChromaDB directory."""
...
def collection_write_roundtrip_works(col) -> bool:
    ...
    probe_doc = "trimemo migrate write round-trip probe"
    probe_meta = {
