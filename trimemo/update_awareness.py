...
_INSTALL_ACTIONS = {
    "uv-tool": ("uv", "tool", "upgrade", "trimemo"),
    "pipx": ("pipx", "upgrade", "trimemo"),
    "pip": None,
}
...
def _installer_action(installer: str) -> dict:
    argv = (
        [sys.executable, "-m", "pip", "install", "--upgrade", "trimemo"]
        if installer == "pip"
...
def fetch_latest_stable() -> str:
    ...
    request = Request(
        "https://pypi.org/pypi/trimemo/json",
        headers={"Accept": "application/json", "User-Agent": "trimemo-update-check"},
    )
    ...
    if not isinstance(version, str) or not _STABLE_VERSION.fullmatch(version):
        raise ValueError("PyPI returned an invalid stable TriMemo version")
    return version
...
def schedule_update_check(
    *, config_dir=None, fetch_latest=fetch_latest_stable, now: datetime | None = None
    ...
    threading.Thread(target=run, name="trimemo-update-check", daemon=True).start()
    return True
