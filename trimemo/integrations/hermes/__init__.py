"""TriMemo memory provider for Hermes.

Implements the Hermes ``MemoryProvider`` ABC (``agent/memory_provider.py``)
so TriMemo can be selected as ``memory.provider: trimemo`` in
``~/.hermes/config.yaml``.
...
* Configuration precedence: ``$HERMES_HOME/mempalace.json`` is read
  ...
  still unset after that defers to trimemo's own config
  ...
  ``collection_name`` follows trimemo's own config for the same
...
* ``~/.mempalace/identity.txt`` (L0) and ``~/.mempalace/wing_config.json``
  ...
  ``trimemo init <project-dir>`` to generate them.
...
TOOL_SCHEMAS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "drawer that supersedes the old one — trimemo is "
            "append-first."
        ),
        "parameters": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            ...
            "wrong filing — trimemo's design prefers superseding adds."
        ),
        "parameters": {
            "type": "object",
...
class MempalaceProvider(MemoryProvider):  # type: ignore[misc]
    """Hermes memory provider backed by TriMemo."""
    ...
    def __init__(self) -> None:
        ...
        # ChromaDB access through trimemo's own backend (matches embedding
    ...
    def is_available(self) -> bool:
        """Always True — module-level imports prove trimemo is installed.

        If trimemo were missing, ``import`` of this module would have failed
    ...
    def initialize(self, session_id: str, **kwargs: Any) -> None:
        ...
        if agent_context in {"cron", "flush"} or platform == "cron":
            logger.debug(
                "TriMemo inactive: agent_context=%s, platform=%s",
                agent_context,
                platform,
            )
        ...
        with self._init_lock:
            ...
            # No hermes-side palace configured → defer to trimemo's own
            # config (env var > ~/.mempalace/config.json > default) so this
            # provider agrees with `trimemo init`-managed setups instead
            ...
            # Collection name follows trimemo's own config — the same
            ...
            try:
                ...
                logger.info(
                    "TriMemo: collection '%s' ready (palace=%s)",
                    self._collection_name,
                    self._palace_path,
                )
                backend_ready = True
            except Exception as exc:
                logger.warning("TriMemo backend init failed: %s", exc)
                with self._collection_lock:
                    self._collection = None
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        try:
            ...
            if not hits:
                return ""
            lines = ["## TriMemo — relevant context"]
            for r in hits:
                wing = r.get("wing", "")
                room = r.get("room", "")
            ...
            logger.debug("TriMemo prefetch error: %s", exc)
            return ""
    ...
    def sync_turn(
        ...
        *,
        ...
        try:
            ...
            # Loud, not silent: the verbatim invariant is what trimemo sells.
            # If the queue saturates we want operators to see it.
            logger.warning(
                "TriMemo worker queue full (maxsize=%d) — turn dropped; "
    ...
    def on_memory_write(
        ...
        try:
            ...
            logger.warning("TriMemo queue full at memory_write — entry dropped")
    ...
    def handle_tool_call(self, tool_name: str, args: Dict[str, Any], **kwargs: Any) -> str:
        if self._cron_skipped:
            return json.dumps({"error": "TriMemo not active (cron context)."})
        if not self._initialized:
            return json.dumps({"error": "TriMemo not initialized."})
        args = args or {}
        try:
            ...
            # trimemo asymmetry to remap: ``mempalace_traverse`` maps to
            ...
            logger.exception("TriMemo tool %s failed", tool_name)
    ...
    # trimemo's admin / internal tool_* functions hidden from this
    ...
    def _dispatch_mcp_passthrough(self, tool_name: str, args: Dict[str, Any]) -> Optional[str]:
        ...
        if tool_name not in self._MCP_PASSTHROUGH_TOOLS:
            return None
        from trimemo import mcp_server as _mp_mcp
    ...
    def save_config(self, values: Dict[str, Any], hermes_home: str) -> None:
        ...
        if config_path.exists():
            try:
                ...
                logger.debug("TriMemo config read failed: %s", exc)
        existing.update(values)
        config_path.write_text(json.dumps(existing, indent=2) + "\n")

    def post_setup(self, hermes_home: str, config: Dict[str, Any]) -> None:
        print()
        print("TriMemo provider installed. To finish setup:")
        print("  1. trimemo init <your-project-dir>     # generates ~/.mempalace/")
    ...
    def shutdown(self) -> None:
        ...
        if self._worker_thread and self._worker_thread.is_alive():
            ...
            if self._worker_thread.is_alive():
                logger.warning("TriMemo worker did not drain within shutdown timeout")
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if self._hermes_home:
            # New name first, legacy name as fallback so pre-rename installs
            # keep resolving their existing config.
            for name in ("trimemo.json", "mempalace.json"):
                config_path = Path(self._hermes_home) / name
                if config_path.exists():
                    try:
                        config.update(json.loads(config_path.read_text()))
                    except Exception as exc:
                        logger.debug("TriMemo config load failed: %s", exc)
                    break
    ...
    def _load_wing_config(self) -> None:
        ...
        if not wing_config_path.exists():
            self._wing_config = {}
            logger.debug("TriMemo: no wing_config.json — run `trimemo init` to configure wings")
            return
        try:
            with open(wing_config_path) as f:
                self._wing_config = (json.load(f) or {}).get("wings", {})
        except Exception as exc:
            logger.warning("TriMemo wing config load failed: %s", exc)
            self._wing_config = {}
    ...
    def _load_identity(self) -> None:
        ...
        try:
            ...
            logger.warning("TriMemo identity load failed: %s", exc)
            self._identity = ""
    ...
    def _refresh_wake_up_cache(self) -> None:
        ...
        try:
            ...
            logger.debug("TriMemo wake-up refresh error: %s", exc)
            self._wake_up_cache = ""
        finally:
    ...
    def _file_turn(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug("TriMemo _file_turn error: %s", exc)
    ...
    def _mirror_mem_write(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug("TriMemo _mirror_mem_write error: %s", exc)
        finally:
            if kg is not None:
                try:
                    kg.close()
    ...
    def _background_worker(self) -> None:
        ...
        while not self._worker_stop.is_set() or not self._worker_queue.empty():
            ...
            try:
                ...
                logger.debug("TriMemo worker task %s error: %s", task, exc)
            finally:
                try:
    ...
    # ----- Tool handlers (delegate to trimemo internals) ----------------
    ...
    def _tool_status(self) -> Dict[str, Any]:
        with self._collection_lock:
            col = self._collection
        if col is None:
            return {"error": "TriMemo not initialized"}
        total = col.count()
    ...
    def _tool_list_rooms(self, wing: str) -> Dict[str, Any]:
        ...
        if truncated:
            ...
            # in the versions trimemo pins, so we can't cheaply give an
    ...
    def _kg_db_path(self) -> str:
        ...
        tools must never fall back to trimemo's global ``DEFAULT_KG_PATH``
    ...
    def _tool_diary_read(self, n: int = 10) -> Dict[str, Any]:
        ...
        for raw_line in tail:
            try:
                ...
                logger.debug("TriMemo: skipping malformed diary line")
...
def register(ctx: Any) -> None:
    """Register the TriMemo memory provider with Hermes."""
