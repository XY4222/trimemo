"""Thin stdio front end for the TriMemo MCP server.

``trimemo-mcp`` is spawned once per agent session, and when a hub is
...
_DEGRADED_NOTICE = (
    "TriMemo is running WITHOUT its shared hub. This session is now serving "
    ...
    "back — check that the TriMemo hub process is alive."
)
...
def _annotate_degraded(response):
    ...
    if not isinstance(content, list):
        return response
    result["content"] = [{"type": "text", "text": f"[trimemo] {_DEGRADED_NOTICE}"}, *content]
    return response
...
class _LocalServer:
    ...
    def load(self):
        if self._module is None:
            logger.warning(
                "TriMemo hub unavailable; serving this session locally. "
