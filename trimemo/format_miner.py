"""format_miner.py — proposed for trimemo 3.3.6.
...
Architecture matches the existing miner-per-content-type pattern:
    ...
    trimemo mine <dir>                  → miner.py
    trimemo mine <dir> --mode convos    → convo_miner.py
    trimemo mine <dir> --mode extract   → format_miner.py (this file)
...
If the user runs ``trimemo mine --mode extract`` without it installed,
...
def scan_formats(directory: Union[Path, str]) -> list[Path]:
    ...
    # CLI inputs like ``trimemo mine --mode extract ~/docs`` work without
...
def mine_formats(
    ...
    try:
        ...
        print(f"\n{'=' * 55}")
        print("  TriMemo Mine -- Format extraction")
        print(f"{'=' * 55}")
        print(f"  Wing:    {wing}")
