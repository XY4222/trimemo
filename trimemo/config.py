...
with back-compat for existing installs:
  ...
  1. $TRIMEMO_CONFIG_DIR (or legacy $MEMPALACE_CONFIG_DIR) if set (explicit override, used by tests and CI)
  2. ~/.mempalace if it already exists (existing installs keep working)
  3. $XDG_CONFIG_HOME/trimemo if XDG_CONFIG_HOME is set
  4. ~/.config/trimemo (XDG default fallback)
"""
...
def _xdg_config_dir(xdg_root: Path) -> Path:
    """Prefer ``trimemo``; fall back to a real legacy ``mempalace`` install.

    Mirrors the ``~/.trimemo`` / ``~/.mempalace`` dual-name pattern: a legacy
    XDG directory (``~/.config/mempalace``) still counts when it looks like a
    real install, so pre-rename users keep their config.json and palace
    resolution without a manual migration.
    """
    new = xdg_root / "trimemo"
    if new.is_dir():
        return new
    legacy = xdg_root / "mempalace"
...
def _default_config_dir() -> Path:
    ...
    """
    env_dir = os.environ.get("TRIMEMO_CONFIG_DIR") or os.environ.get("MEMPALACE_CONFIG_DIR")
    if env_dir and env_dir.strip():
        return Path(env_dir).expanduser()

    new_dir = Path.home() / ".trimemo"
    if new_dir.is_dir():
        return new_dir
    ...
    if xdg and xdg.strip():
        ...
        if xdg_path.is_absolute():
            return _xdg_config_dir(xdg_path)

    return _xdg_config_dir(Path.home() / ".config")
...
_LEGACY_PALACE_PATH = os.path.expanduser("~/.mempalace/palace")
DEFAULT_PALACE_PATH = (
    os.path.expanduser("~/.trimemo/palace")
    if os.path.isdir(os.path.expanduser("~/.trimemo"))
    else _LEGACY_PALACE_PATH
)
...
# pruned. Applies to the accumulating backups written by ``trimemo
# migrate`` and ``trimemo repair max-seq-id`` — see
...
class MempalaceConfig:
    """Configuration manager for TriMemo.
    ...
    def palace_path(self):
        ...
        env_val = os.environ.get("TRIMEMO_PALACE_PATH") or os.environ.get("MEMPALACE_PALACE_PATH") or os.environ.get("MEMPAL_PALACE_PATH")
        if env_val:
    ...
    def backend(self):
        ...
        env_val = os.environ.get("TRIMEMO_BACKEND") or os.environ.get("MEMPALACE_BACKEND")
        if env_val:
    ...
    def pgvector_dsn(self):
        ...
        if env_val:
            return env_val.strip()
        return str(
            self._file_config.get("pgvector_dsn", "postgresql://localhost:5432/trimemo")
        ).strip()
    ...
    def embedding_model(self):
        ...
        EF name doesn't match. Run ``trimemo repair rebuild-index`` after
        changing this value.
        """
    ...
    def max_backups(self) -> int:
        ...
        ``trimemo migrate`` (``<palace>.pre-migrate.<timestamp>``) and
        ``trimemo repair max-seq-id``
