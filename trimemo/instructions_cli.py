"""
Instruction text output for TriMemo CLI commands.
...
_RULES_MARKER_START = (
    "<!-- trimemo-shared-brain:start (canonical source: trimemo repo "
    "integrations/shared/coordination-protocol.md — edit there, re-render "
    "with `trimemo rules --host {host} --harness {harness} --project {project}`) -->"
)
_RULES_MARKER_END = "<!-- trimemo-shared-brain:end -->"
...
def validate_identity_component(name: str, value: str) -> str:
    ...
    if not _COMPONENT_RE.fullmatch(value):
        raise ValueError(
            ...
            "trimemo (letters, digits, '.', '_', '-'; no colons, no uppercase)"
        )
    return value
