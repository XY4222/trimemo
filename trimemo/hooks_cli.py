"""
Hook logic for TriMemo — Python implementation of session-start, stop, session-end, and precompact hooks.
...
``transcript_path``. The TriMemo DSH plugin (``.dsh-plugin/``) therefore keeps
...
def _palace_root_exists() -> bool:
    ...
    directory (``~/.config/trimemo`` by default) and never creates
...
def _mempalace_python() -> str:
    """Return the python interpreter that has trimemo installed.
    ...
    3. Editable install: venv/ sibling to trimemo/
    ...
    if env_python and os.path.isfile(env_python) and os.access(env_python, os.X_OK):
        return env_python
    # This file lives at <venv>/lib/pythonX.Y/site-packages/trimemo/hooks_cli.py
    # or <project>/trimemo/hooks_cli.py (editable install).
    #
    ...
    if len(parents) > 3:
        ...
        if venv_bin.is_file():
            return str(venv_bin)
    # Editable install: assumes project root has a venv/ sibling to trimemo/
    if len(parents) > 1:
...
PRECOMPACT_BLOCK_REASON = (
    "TriMemo emergency save — compaction imminent. "
...
def _output(data: dict):
    ...
    mcp_mod = sys.modules.get("trimemo.mcp_server") or sys.modules.get(
...
class HookWriteRouting:
    ...
    def notice(self) -> str:
        if self.error is not None:
            return (
                "TriMemo hook writes were skipped because write-routing "
                f"configuration is invalid: {self.error}. No direct ChromaDB "
                "fallback was attempted."
            )
        if self.blocked:
            return (
                "TriMemo hook writes were skipped because routing is set to "
                "'require' but the local daemon is unavailable. Start it with "
                "`trimemo daemon start`; no direct ChromaDB fallback was attempted."
            )
        return ""
        ...
        try:
            if routing.use_daemon:
                try:
                    _submit_daemon_job(
                        "mine",
                        {"source": mine_dir, "mode": mode, "agent": "trimemo"},
                ...
                continue
            _spawn_mine([_mempalace_python(), "-m", "trimemo", "mine", mine_dir, "--mode", mode])
        except OSError:
            pass
        except Exception as exc:
...
def _desktop_toast(body: str, title: str = "TriMemo"):
    """Send a desktop notification via notify-send. Fails silently."""
    try:
        subprocess.Popen(
            ["notify-send", "--app-name=TriMemo", "--icon=brain", title, body],
