...
* ``embeddinggemma`` — ``onnx-community/embeddinggemma-300m-ONNX`` (q8), 384-dim
  ...
  on an existing palace requires ``trimemo repair rebuild-index``
...
* ``openai-compat`` — embeddings served by any OpenAI-compatible
  ...
  switching to/from this backend also requires ``trimemo repair
...
* ``cuda`` — NVIDIA GPU via ``onnxruntime-gpu`` (``pip install trimemo[gpu]``)
...
def _resolve_providers(device: str, model: Optional[str] = None) -> tuple[list, str]:
    ...
    if preferred not in available:
        if device not in _WARNED:
            extra = _DEVICE_EXTRA.get(device, "the matching trimemo extra for your device")
            logger.warning(
...
_EMBEDDINGGEMMA_WITNESS = "trimemo embedding provider health check"
...
class EmbeddinggemmaONNX:
    ...
    and ChromaDB rejects mismatched reads. Run ``trimemo repair rebuild-index``.
    """
        ...
        with self._load_lock:
            ...
            try:
                ...
                raise ImportError(
                    ...
                    "numpy — these ship with trimemo core, so this error usually "
                    "means one was uninstalled or pinned to an incompatible version. "
                    "Reinstall with: pip install --upgrade --force-reinstall trimemo"
                ) from e
...
class OpenAICompatEmbeddingFunction:
    ...
    collection and rejects mismatched reads, the signal to run ``trimemo
    ...
    def __call__(self, input):  # noqa: A002 — ChromaDB EF protocol uses `input`
        ...
        headers = {
            ...
            "User-Agent": f"trimemo/{__version__}",
        }
        if self._api_key:
