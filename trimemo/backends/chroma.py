"""ChromaDB-backed TriMemo storage backend (RFC 001 reference implementation)."""
...
def hnsw_capacity_status(palace_path: str, collection_name: str = "mempalace_drawers") -> dict:
    ...
    * ``message``          — human-readable summary
    ...
    wall-clock TTL, so an external writer — ``trimemo repair``, a peer mine,
...
def _hnsw_capacity_status_uncached(
    ...
    try:
            ...
            out.update(
                {
                    ...
                    "message": (
                        ...
                        f"(max={binary_header['max_elements']:,}, "
                        f"current={binary_header['cur_element_count']:,}). "
                        "Vector reads are disabled until `trimemo repair` "
                        "rebuilds the index."
                    ),
                }
            )
        ...
        # Newer palaces explicitly store trimemo's low sync threshold
...
def _fix_blob_seq_ids(palace_path: str) -> None:
    ...
    ``trimemo repair --mode max-seq-id``.
...
class ChromaCollection(BaseCollection):
    ...
    direct-backend writers against ``trimemo mine`` and against each other,
    ...
    def lexical_search(
        ...
        *,
        ...
        # TriMemo paths above use Chroma's FTS table instead of scanning every
...
class ChromaBackend(BaseBackend):
    """TriMemo's default ChromaDB backend.
    ...
    * An inode+mtime freshness check absorbed from ``mcp_server._get_client``
    ...
    def _explain_ef_mismatch(error: Exception, palace_path: str) -> Optional[str]:
        ...
        rebuild_cmd = f"trimemo --palace {shlex.quote(palace_path)} repair rebuild-index"
        return (
