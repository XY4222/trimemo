"""
logstream.py — Agent coordination event log for TriMemo (RFC 003)
===================================================================
...
and exchange exact patch/file artifacts through the shared TriMemo hub
...
Usage:
    ...
    evt = ls.append_event(
        type="task.request", stream="project/trimemo", room="delegation",
...
def _sanitize_routing(value, field_name: str, required: bool = True) -> Optional[str]:
    ...
    Streams may contain ``/`` (``project/trimemo``), so this is looser
...
def sanitize_watch_state_basename(agent: str) -> str:
    ...
    RFC 005 identities use colons (``windows:grok:trimemo``), which are
