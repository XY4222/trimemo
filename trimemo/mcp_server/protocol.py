...
def _mcp_diverged_index_refusal(req_id, tool_name: str):
    ...
    ``trimemo repair rebuild-index`` all five committed.
    ...
    return {
        ...
        "error": {
            ...
            "data": {
                ...
                "hint": (
                    "Stop the TriMemo MCP servers, run `trimemo repair rebuild-index`, "
...
def _watched_metadata_files(root: str) -> list[str]:
    ...
    for dist in _STALE_LIBRARY_WATCHED_DISTS:
        ...
        for entry in entries:
            ...
            # escaping already spells a sibling like `trimemo-remote` as
...
# already in sys.modules by now — trimemo by definition, chromadb through the
...
def _mcp_tool_preflight_refusal(req_id, tool_name: str, *, check_writer: bool = True):
    ...
    # `trimemo repair rebuild-index`, which would execute the INSTALLED code
    ...
    if method == "initialize":
        ...
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "trimemo", "version": __version__},
            },
        }
    elif method == "ping":
...
def _maybe_eager_warmup_embedder() -> None:
    ...
    ``trimemo init`` (which would violate CLAUDE.md "Incremental only"),
    ...
    config and launch the server before running ``trimemo init`` get a
    ...
    * **Query failure** (network failure during ONNX model download,
    ...
    ``trimemo init`` but never written to), ``col.query`` succeeds but
        ...
        while True:
            ...
            if action == "warn":
                logger.warning(
                    ...
                    "read-only. Check `trimemo repair --dry-run` for HNSW divergence; "
