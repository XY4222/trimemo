...
class _MempalaceLogFilter(logging.Filter):
    """Pass only records emitted by trimemo's own loggers.
    ...
    host's — or a third-party library's — records into trimemo's diagnostic
    file. trimemo loggers are ``trimemo`` / ``mempalace.*`` (the dotted
    ``__name__`` family) plus the flat ``mempalace_mcp`` /
    ``mempalace_format_miner`` / ``mempalace_hallways`` / ``mempalace_graph``
    loggers — every one is prefixed ``trimemo``.
    """

    def filter(self, record: logging.LogRecord) -> bool:
        name = record.name
        return name == "trimemo" or name.startswith(
            ("trimemo.", "mempalace.", "mempalace_")
        )
...
def _init_logging() -> None:
    """Configure trimemo logging: stderr by default, optional file append.
    ...
    * **Root unconfigured** (standalone ``trimemo-mcp``): own it — a stderr
    ...
    * **Root already configured** (an app imported ``mempalace.mcp_server``
      ...
      trimemo's own records (`_MempalaceLogFilter`), so the host's logs do
      not bleed into trimemo's file. With ``MEMPALACE_LOG_FILE`` unset the
    ...
    The file handler is trimemo-filtered in both paths, so the file is a
    clean trimemo-only stream. In the embedded path trimemo's records are
    ...
    * Concurrent writers (multiple ``trimemo-mcp`` processes at one path)
    ...
    if log_file:
        try:
            ...
            # File is a trimemo-only diagnostic stream; keep host / library
