# Loaded into trimemo.mcp_server via exec (see __init__.py). Not a standalone module.
if __name__ != "trimemo.mcp_server":
    raise ImportError(f"{__name__} is an implementation fragment; import trimemo.mcp_server")
...
def _parse_args():
    parser = argparse.ArgumentParser(description="TriMemo MCP Server")
    parser.add_argument(
        "--palace",
        metavar="PATH",
...
# the *startup* probe when the database exceeds this size (MB). `trimemo
...
#
# A long-lived MCP server imports trimemo and its storage backend once and
# then serves from those in-memory modules for the whole life of the process.
# Upgrading the package on disk mid-session (`pip install -U trimemo`,
...
#
# #457 is the same condition with the volume turned up: upgrading trimemo
...
# server to stop first; `trimemo migrate` (#502) only recovers such a palace
# after the fact.
#
# Watched distributions are the ones whose code writes the palace on this
# machine: trimemo itself, plus chromadb when chromadb is the backend
...
def _acquire_mcp_writer_lock() -> tuple[bool, str]:
    ...
    try:
        ...
        _MCP_WRITER_LOCK_ERROR = (
            "another trimemo writer already holds the palace lock for "
    ...
    if max_bytes > 0:
        ...
        if db_bytes > max_bytes:
            ...
            _sqlite_integrity_no_verdict_reason = (
                ...
                "against this palace. Run `trimemo repair` for a full check."
            )
            logger.warning(
                "SQLite startup integrity check skipped: %s is %s MB "
                "(> %s MB limit); PRAGMA quick_check would block MCP "
                "startup. Run `trimemo repair` for a full check, or set "
    ...
    return {
        ...
        "error": {
            ...
            "data": {
                ...
                "sqlite_version": sqlite3.sqlite_version,
                "hint": (
                    "Stop all TriMemo MCP clients/writers, back up the palace, "
...
def _resolve_logstream_path() -> str:
    ...
    if not palace_path:
        raise ValueError("no palace path configured; run trimemo init first")
    ...
    if info.get("diverged"):
        if not _vector_disabled:
            logger.warning(
                ...
                "BM25-only sqlite fallback. Run `trimemo repair` to restore "
                "vector search.",
