...
# Stdio→hub proxying. When a long-lived HTTP hub (`trimemo serve`) owns
...
def _run_stdio_loop() -> None:
    ...
    logger.info("TriMemo MCP Server starting...")
...
def _run_http_loop() -> None:
    ...
    logger.info("TriMemo MCP HTTP server starting...")
...
def main():
    """MCP server entry point for the ``trimemo-mcp`` console script.
    ...
    # with a clean env. The sys.path filter in trimemo/__init__.py
