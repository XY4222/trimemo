...
def _start_peer_sync_thread(
    ...
    thread = threading.Thread(target=_loop, name="trimemo-logsync", daemon=True)
...
def _serve_http(host: str, port: int) -> None:
    ...
    long-lived stdout pipe for operators who run TriMemo behind an HTTP MCP
    ...
    # (save hooks, plain `trimemo mine`) forward their writes here instead
    ...
    if not _http_is_loopback(host):
        if httpd.auth_token:
            logger.warning(
                "TriMemo MCP HTTP server bound to non-loopback host %s; /mcp "
                "requires the configured bearer token.",
                host,
            )
        else:
            logger.warning(
                "TriMemo MCP HTTP server bound to non-loopback host %s without "
    ...
    with httpd:
        logger.info(
            "TriMemo MCP HTTP server listening on %s://%s:%s/mcp%s%s",
        ...
        try:
            ...
            logger.info("TriMemo MCP HTTP server shutting down")
        finally:
