...
PALACE_PROTOCOL = """IMPORTANT — TriMemo Memory Protocol:
...
AAAK_SPEC = """AAAK is a compressed memory dialect that TriMemo uses for efficient storage.
...
def tool_check_duplicate(content: str, threshold: float = 0.9):
    ...
    if _vector_disabled:
        ...
        return {
            ...
            "vector_disabled_reason": _vector_disabled_reason,
            "hint": (
                "duplicate detection requires vector search; run `trimemo repair` to restore"
            ),
        }
    col = _get_collection()
    if not col:
