...
- If MCP is not available: Run the CLI command: trimemo status
...
- Empty palace (zero memories): Suggest "Try /trimemo:mine to add data from
  files, URLs, or text."
- Has data but no knowledge graph (memories exist but KG stats show zero
  ...
  queries."
- Healthy palace (has memories and KG data): Suggest "Use /trimemo:search to
  query your memories."
