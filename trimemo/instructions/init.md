...
Guide the user through a complete TriMemo setup. Follow each step in order,
...
## Step 2: Check if trimemo is already installed

Run `trimemo --version`. If it succeeds, the CLI is on PATH — report
the installed version and skip to Step 4.

If `trimemo --version` fails, **do not** skip to Step 4 just because
`pip show trimemo` or `uv tool list` reports the package as installed:
the package may live inside a venv that isn't activated, in which case
Step 5 (`trimemo init ...`) will fail with `command not found`. Treat
...
## Step 3: Install trimemo

Prefer [`uv`](https://docs.astral.sh/uv/) — it isolates the CLI from system
Python and avoids most environment-related failures:

1. If `uv` is on PATH (`uv --version`), run `uv tool install trimemo`.
2. Otherwise run `pip install trimemo`.

### Error handling -- install failures

If the install command fails, try these fallbacks in order:

1. If `uv tool install` failed, try `pip install trimemo` (or vice versa).
2. Try `pip3 install trimemo`.
3. Try `python -m pip install trimemo` (or `python3 -m pip install trimemo`).
...
Ask the user which project directory they want to initialize with TriMemo.
Offer the current working directory as the default. Wait for their response
before continuing.

## Step 5: Initialize the palace

Run `trimemo init --yes <dir>` where `<dir>` is the directory from Step 4.
...
Run `trimemo status` and confirm the output shows a healthy palace.
...
- Use /trimemo:mine to start adding data to their palace
- Use /trimemo:search to query their palace and retrieve stored knowledge
