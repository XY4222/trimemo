...
| Command              | Description                    |
|----------------------|--------------------------------|
| /trimemo:init      | Install and set up TriMemo   |
| /trimemo:search    | Search your memories           |
| /trimemo:mine      | Mine projects and conversations|
| /trimemo:status    | Palace overview and stats      |
| /trimemo:help      | This help message              |
...
## CLI Commands
    ...
    trimemo init <dir>                  Initialize a new palace
    trimemo mine <dir>                  Mine a project (default mode)
    trimemo mine <dir> --mode convos    Mine conversation exports
    trimemo search "query"              Search your memories
    trimemo split <dir>                 Split large transcript files
    trimemo wake-up                     Load palace into context
    trimemo compress                    Compress palace storage
    trimemo status                      Show palace status
    trimemo repair                      Rebuild vector index
    trimemo mcp                         Show MCP setup command
    trimemo hook run                    Run hook logic (for harness integration)
    trimemo instructions <name>         Output skill instructions
...
Hooks read JSON from stdin and output JSON to stdout. They can be invoked via:
    ...
    echo '{"session_id":"abc","stop_hook_active":false,"transcript_path":"..."}' | trimemo hook run --hook stop --harness claude-code
...
1. /trimemo:init -- Set up your palace
2. /trimemo:mine -- Mine a project or conversation
3. /trimemo:search -- Find what you stored
