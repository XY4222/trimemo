"""Long-lived local daemon for queued TriMemo writes.
...
class DaemonRuntime:
    ...
    def start_worker(self) -> threading.Thread:
        ...
        thread = threading.Thread(
            target=self._worker_loop, name="trimemo-daemon-worker", daemon=True
        )
...
def _close_writer_lease_after_worker(
    ...
    threading.Thread(
        ...
        name="trimemo-daemon-writer-lease-reaper",
        daemon=True,
    ).start()
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="TriMemo daemon internals")
