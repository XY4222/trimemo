---
name: trimemo-recall
description: "Recall protocol for TriMemo — search the palace before answering about past work, people, projects, or prior decisions. Apply when the user asks what was decided, what happened before, who someone is, what was discussed last time, or anything that may already be filed in their memory palace; or when trimemo-recall is invoked. Complements the trimemo setup skill and requires the trimemo-mcp server."
---

# TriMemo Recall

Search-before-answer protocol for TriMemo. This skill makes the agent
...
the `trimemo` skill, which covers install / mine / status; this one
covers recall only.

## Step 0 — Verify TriMemo is available
...
server is not connected and point them at the `trimemo` skill or
`/trimemo-init` to set it up. Do not silently fall back to answering
from model memory.
...
and violates TriMemo's "memory should feel instant" budget.
...
- **MCP error / server down.** Surface the error and suggest the user
  run `trimemo status` or re-run `/trimemo-init`. Never fall back to
  guessing.
